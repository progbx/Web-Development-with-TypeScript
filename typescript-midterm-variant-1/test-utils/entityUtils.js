...
function isEntityItemFitName({ item, entityName, identifierPropKey }) {
    let itemBody = normalizeEntity(item);

    if (itemBody.type === AST_NODE_TYPES.VariableDeclaration) {
        return itemBody.declarations[0][identifierPropKey].name === entityName;
    }

    return itemBody[identifierPropKey]?.name === entityName;
}

export function getRootLevelEntityListByName({
    parentEntity,
    entityName,
    ...
    const entities = body.filter((item) =>
        isEntityItemFitName({ item, entityName, identifierPropKey })
    );
...
export function isEntityFunction(functionTestEntity) {
    return (
        !!getFunctionEntityBody(functionTestEntity) ||
        isMethod(functionTestEntity)
    );
}
...
export function isOmitType({
    ...
    const expectedOmittedPropertiesSorted = expectedOmittedProperties.sort();
    ...
    const realOmittedProperties = secondTypeParam.types
        ? secondTypeParam.types.map((typeItem) => typeItem.literal.value).sort()
    ...
    return (
        typeName === "Omit" && isFirstTypeParamValid && isSecondTypeParamValid
    );
}

export function getIntefacePropertiesSpecs(propertyEntities, propertyNames) {
    return propertyNames.map((itemName) => {
        ...
        const rootTypeAnnotation = propertyEntity.typeAnnotation || {};

        return {
            type: rootTypeAnnotation?.typeAnnotation?.type,
            optional: propertyEntity?.optional,
        };
    });
}
