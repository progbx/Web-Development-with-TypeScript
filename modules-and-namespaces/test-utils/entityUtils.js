...
export function getRootLevelVariableByName({
    parentEntity,
    entityName,
}) {
    ...
    const entity = body.find((item) => {
        ...
        const [declarator] = itemBody.declarations;

        return declarator.id.name === entityName;
    });

    return normalizeEntity(entity);
}

export function getVariableDeclarator(variableDeclarationEntity) {
    if (!variableDeclarationEntity) {
        return null;
    }

    return variableDeclarationEntity.declarations[0];
}
...
export function isMethod(functionTestEntity) {
    return (
        functionTestEntity.type === AST_NODE_TYPES.TSMethodSignature ||
        functionTestEntity.type === AST_NODE_TYPES.MethodDefinition
    );
}
